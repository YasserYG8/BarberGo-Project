...
export default function HairdresserProfile() {
  ...
  const [hairdresserId, setHairdresserId] = useState<number | null>(null);
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const [userProfile, hairdresserProfile] = await Promise.all([
          getMyUserProfile(),
          getMyHairdresserProfile(),
        ]);
        ...
        setProfileForm({
          ...
          bio: hairdresserProfile.bio || "",
          specialty: hairdresserProfile.specialty || "",
  ...
  const submitProfileUpdate = async (event: React.FormEvent) => {
    ...
    if (!hairdresserId) {
      toast("Unable to resolve hairdresser profile", "error");
      return;
    }

    setProfileSubmitting(true);
    try {
      const [updatedUser, updatedHairdresser] = await Promise.all([
        updateMyUserProfile({
          fullName: profileForm.fullName,
          phone: profileForm.phone || undefined,
          address: profileForm.address || undefined,
        }),
        updateMyHairdresserProfile(hairdresserId, {
      ...
      setProfileForm((prev) => ({
        ...prev,
        bio: updatedHairdresser.bio || "",
        specialty: updatedHairdresser.specialty || "",
      }));
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <div>
        ...
        <p className="text-stone-400 mt-1">Update your contact and professional details.</p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Your Information</CardTitle>
          <CardDescription>This information will be displayed to clients.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <form className="grid gap-4 sm:grid-cols-2" onSubmit={submitProfileUpdate}>
            ...
            <div className="sm:col-span-2 mt-4">
              <Button type="submit" disabled={profileSubmitting || !hairdresserId}>
